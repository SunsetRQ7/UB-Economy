...
public class WithdrawCommand implements CommandExecutor {
    ...
    public WithdrawCommand(UBEconomyPlugin plugin, EconomyService economyService) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length != 1) {
                Message.send(player, "&cEstefade: &7/withdraw <meghdar> ⚠️");
                return true;
            }
            ...
            try {
                ...
                economyService.withdraw(player, amount);
